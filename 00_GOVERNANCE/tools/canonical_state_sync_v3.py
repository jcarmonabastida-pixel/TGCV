...
def audit_matrix(predecessor,successor):
 ...
 if not evidence_sections(predecessor)<=evidence_sections(successor): e.append("SUCCESSOR_LOSES_EVIDENCE_SECTIONS")
...
def resolve_manifest(matrix_version,source_commit):
 ...
 for path in FILES:
  ...
  if source_kind=="source_commit_canonical" and current_bytes!=target_bytes: errors.append(f"CANONICAL_SOURCE_MISMATCH:{name}")
