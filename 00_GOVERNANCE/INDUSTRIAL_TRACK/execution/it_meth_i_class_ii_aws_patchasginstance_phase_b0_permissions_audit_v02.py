"""IT-METH-I Class II AWS-PatchAsgInstance — B0 permissions evidence audit v0.2.

Correction over v0.1:
  Frozen role ARN presence is NOT treated as role availability. Availability is
  PASS only when IAM GetRole positively resolves the frozen principal(s).
...
SCHEMA = "IT-METH-I-CLASS-II-AWS-PATCHASGINSTANCE-PHASE-B0-PERMISSIONS-AUDIT-2"
...
def main() -> int:
    ...
    frozen_roles = {
        "AutomationAssumeRole": {"arn": frozen_automation_role, "role_name": role_name_from_arn(frozen_automation_role), "status": "UNRESOLVED"},
        "LambdaRoleArn": {"arn": frozen_lambda_role, "role_name": role_name_from_arn(frozen_lambda_role), "status": "UNRESOLVED"},
    }
    ...
    # IAM role availability is a PASS only when every frozen candidate role is positively resolved.
    if any_frozen_role and all(info["status"] == "PASS" for info in frozen_roles.values() if info["arn"]):
        ...
        iam_role_status = "UNRESOLVED"
        iam_role_reason = "No frozen candidate role ARN is available in Phase-A evidence."

    # Read-only policy attachment inventory for resolved frozen roles.
    ...
    # Never collapse simulation failure or incomplete service-role semantics into PASS.
    ...
    result = {
        ...
        "common_boundary": {"phase_a_record": str(args.phase_a_record), "target_instance_id": instance_id, "asg_name": asg_name, "postdecision_outcomes_used": False},
        "frozen_candidate_parameters": {"AutomationAssumeRole": frozen_automation_role, "LambdaRoleArn": frozen_lambda_role, "retrofit_from_discovered_roles": False},
        "permissions_audit": {
            "iam_role_availability": {"status": iam_role_status, "reason": iam_role_reason, "frozen_roles": frozen_roles, "resolution_errors": role_resolution_errors, "resolved_role_policies": resolved_role_policies},
            "operation_permission_availability": {"status": operation_status, "reason": operation_reason, "caller_arn": caller_arn, "comparator_simulation": comparator_simulation, "candidate_role_simulations": candidate_role_simulations},
        },
        ...
        "role_inventory_observation": {
            ...
            "matching_role_names_seen": [r.get("RoleName") for r in (roles or {}).get("Roles", []) if isinstance(r, dict) and r.get("RoleName") and any(tok in r.get("RoleName", "").lower() for tok in ("patch", "ssm", "automation", "lambda", "tgcv"))],
        },
        "raw_read_only_probes": raw,
        "probe_errors": errors,
        "guard": {"candidate_invoked": False, "comparator_invoked": False, "ssm_send_command_invoked": False, "automation_start_invoked": False, "iam_mutated": False, "fixture_mutated": False},
        "interpretation": {"permission_equivalence_established": False, "accessibility_superiority_established": False, "industrial_utility_assessed": False, "class_ii_to_class_i_promotion": False},
        "artifact": {"sha256": None, "sha256_basis": "canonical JSON with artifact.sha256=null"},
    }

    args.output_dir.mkdir(parents=True, exist_ok=True)
    out = args.output_dir / "IT-METH-I_CLASS_II_AWS_PATCHASGINSTANCE_PHASE_B0_PERMISSIONS_AUDIT_RESULT_002.json"
