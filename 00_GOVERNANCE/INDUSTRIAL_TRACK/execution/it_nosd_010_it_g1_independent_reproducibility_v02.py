#!/usr/bin/env python3
"""IT-NOSD-010 IT-G1 independent reproducibility executor v0.2.

Corrected against the canonical IT-NOSD-010 CSV schema: timestamp_iso,
from_cell_id/to_cell_id, from_rat/to_rat, operator_code, network_tech and
cell_id. Technical executor only; no industrial/3GPP action is performed.
"""
...
def main() -> int:
    ...
    source_observations = [
        ...
        if norm(r.get("operator_code")) == "310260"
        and norm(r.get("network_tech")) == "5G"
        and norm(r.get("cell_id")) == SOURCE_CELL
    ]

    target_observed_pass = len(target_observations) > 0
    source_state_pass = len(source_observations) > 0
    accessibility_pass = (
        provenance_pass and event_identity_pass and
    ...
    if event_identity_pass:
        e = target_events[0]
        transition_pass = (
            norm(e.get("from_cell_id")) == SOURCE_CELL
            and norm(e.get("to_cell_id")) == TARGET_CELL
            and norm(e.get("from_rat")) == "5G"
            and norm(e.get("to_rat")) == "5G"
            and norm(e.get("from_node")) == NODE
            and norm(e.get("to_node")) == NODE
        )

    # No post-event row is inspected or used for accessibility.
    post_event_rows_used_for_accessibility = 0
    predicates = {
        ...
        "G1-05_accessibility_outcome_separation": accessibility_pass and post_event_rows_used_for_accessibility == 0,
        "G1-06_state_transition_closure": transition_pass,
        "G1-07_temporal_closure": PRE_START < EVENT_TS,
        "G1-08_downstream_separation": post_event_rows_used_for_accessibility == 0,
    ...
    result = {
        "EXECUTOR": "IT-NOSD-010-IT-G1-INDEPENDENT-REPRODUCIBILITY-v0.2",
        ...
        "WINDOW": {
            ...
            "post_event_rows_used_for_accessibility": post_event_rows_used_for_accessibility,
        },
        "PRE_OUTCOME": {
            "target_observation_count": len(target_observations),
            "first_target_observation": min((norm(r.get("timestamp_iso")) for r in target_observations), default=None),
        ...
        "SCHEMA": {
            ...
            "event_source": "from_cell_id",
            "event_target": "to_cell_id",
            "event_RAT": "from_rat/to_rat",
            "spectrum_timestamp": "timestamp_iso",
            "spectrum_operator": "operator_code",
            "spectrum_technology": "network_tech",
            "spectrum_cell": "cell_id",
        },
        "ENVIRONMENT": {
            "python": sys.version,
    ...
    out = OUT / "IT_NOSD_010_IT_G1_INDEPENDENT_REPRODUCIBILITY_RESULT_002.json"
