"""IT-METH-I Class II AWS-PatchAsgInstance — independent reconstruction executor v0.2.

Repair of v0.1: artifact SHA-256 is now a canonical content hash. The hash is
computed over a copy whose artifact.sha256 field is null, then the final artifact
is written once with that digest. This avoids self-referential hash drift.
...
def reconstruct(aws: str, region: str, asg: str, instance_id: str, evidence_dir: Path) -> dict[str, Any]:
    ...
    baseline_details = run_aws(aws, ["ssm", "get-patch-baseline", "--baseline-id", baseline_id], region) if baseline_id else {}
    ...
    observe("health_check_configuration", {"health_check_type": group.get("HealthCheckType"), "health_check_grace_period": group.get("HealthCheckGracePeriod")})
    ...
    state: dict[str, Any] = {
        ...
        "state_vector": {
            ...
            "os_identity": {"platform_details": instance.get("PlatformDetails"), "ssm_platform_type": ssm_info[0].get("PlatformType") if ssm_info else None, "ssm_platform_name": ssm_info[0].get("PlatformName") if ssm_info else None, "ssm_platform_version": ssm_info[0].get("PlatformVersion") if ssm_info else None},
        ...
        "candidate_transformation": {"identity": CANDIDATE, "execution": "NOT_AUTHORIZED"},
        "comparator_transformation": {"identity": "SSM-AWS-RunPatchBaseline/Install on the same frozen target", "execution": "NOT_AUTHORIZED"},
        "independence_control": {"primary_reconstruction_available_to_executor_2": False, "primary_answers_loaded": False, "comparison_results_loaded": False, "postdecision_outcomes_used_to_define_state": False},
    ...
    state["artifact"]["sha256"] = digest
    path.write_bytes(canonical_json(state))
    return {**state, "artifact": {**state["artifact"], "path": str(path), "size": path.stat().st_size}}
...
def main() -> int:
    ...
    print(f"ACCESSIBILITY_PREDICATES_ALL_PASS={all(state['accessibility_predicates'].values())}")
    ...
    print("PRIMARY_RECONSTRUCTION_USED=False")
