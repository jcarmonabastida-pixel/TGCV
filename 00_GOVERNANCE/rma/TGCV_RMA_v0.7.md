# TGCV — Registro Maestro de Activos (RMA) v0.7

**Estado:** CURRENT / OPERATIVE  
**Fecha:** 2026-09-09  
**Predecesor:** `00_GOVERNANCE/rma/TGCV_RMA_v0.6.md`  
**Governance basis:** DR-044 + DR-045 + EXT-UPD-1R.4 + EXT-UPD-3.1 + EXT-UPD-3.2 + EXT-UPD-3.3.5 + EXT-UPD-3.4  
...
**D-OPS-24 is the next controlled operation and remains UNBLOCKED FROM RMA GOVERNANCE CONSISTENCY PERSPECTIVE.**
...
- `TGCV-EXT-RII-001` → `05_ASSETS/RII/` — reserved/identified-pending
- `TGCV-EXT-MOI-001` → `05_ASSETS/MOI/` — reserved; substantive creation deferred

Programme/application adaptations remain subordinate derived artefacts.

### Current content state

`TGCV-EXT-RP-001_v0.1.md` is the current generic Research Prospectus controlled draft. It derives from the current architecture, current claim/evidence boundary, TR-131, bounded RUST-DYN-1/2 evidence and D-OPS-21/22/23. It does not upgrade epistemic status or gate state.

`TGCV-EXT-TCP-001_v0.3.md` is the current Testable Concept Paper controlled draft. It does not upgrade epistemic status or gate state.

`TGCV-EXT-VP-001_v0.2.md` is the current Vision Paper controlled draft. It does not upgrade epistemic status or gate state.

`TGCV-EXT-ARM-001_v0.1.md` is the first substantive canonical Architectural Research Map. It consolidates the current accepted architecture and governance boundaries without adding a scientific claim or empirical result.

Historical/preparatory copies retain their historical status and do not compete with the canonical external identity.
...
- Current traceability: `00_GOVERNANCE/rma/TGCV_RMA_traceability_v0.7.csv`

## 14. Current reconciliation status

**RMA v0.7 CURRENT / OPERATIVE — EXT-UPD-3.4 ARM propagation in progress.**

EXT-UPD-3.4 records the creation and controlled propagation of ARM v0.1. No scientific claims, evidence levels or gate states are changed.

The RMA governance chain is not considered closed until the current pointer, traceability, STATUS, CHANGELOG, validator target and consistency closure are mutually consistent.
