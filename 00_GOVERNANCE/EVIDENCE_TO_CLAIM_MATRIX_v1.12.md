# TGCV — Evidence-to-Claim Matrix — Current v1.12

**Status:** GOVERNANCE CONTROL ARTIFACT — CURRENT  
**Date:** 2026-09-16  
**Predecessor:** v1.11  
**Incremental governance update:** v1.12 preserves the complete material evidentiary content and schema of v1.11 and adds the MT4 bounded methodological domain-transfer evidence. MT4 is propagated as bounded methodological evidence/qualification to C01, C02, C07, C08, C10, C11 and C16, with explicit non-positive propagation to C09 and no claim-level status upgrade. No evidence is deleted, collapsed, or downgraded.

**MT4 update:** MT4 closes as `BOUNDED PASS — METHODOLOGICAL TRANSFER DEMONSTRATED WITH EXPLICIT LIMITS` in a heterogeneous historical electricity-system domain. MT4-1, MT4-2, MT4-3, MT4-4, MT4-6, MT4-7 and MT4-8 passed; MT4-5 remains `OPEN / BOUNDED` because full `P_tau` is not established. The evidence preserves separation among candidate technical constraints, realized transformations, state/trajectory variables and economic/value-related variables. It does not establish full `P_tau`, a downstream TGCV value endpoint, transversal causal validity, or any claim-level upgrade.
...
|---|---|---|---|---|---|
| C01 | TGCV represents system state/context/conditions and constraints/resources | E0 | Formal architecture + bounded cross-domain traces + IT-G1 bounded industrial case + C10C-004 + MT4 | IT-G1 adds material bounded evidence for reconstruction of internal state and external enabling conditions. C10C-004 adds bounded methodological evidence that an empirical construct can be traced from instrument semantics through physical variables to an executable derived outcome, without establishing TGCV state representation. MT4 adds bounded methodological support that a technical constraint/state layer can be identified and preserved in a heterogeneous electricity-system domain. No claim-level upgrade. | Further independent operational confirmation |
| C02 | Accessibility is represented by transformations satisfying an independently defined admissibility predicate | E0 | Formalization + Rust + C-01 A-C + I-01 Gate C + IT-NOSD-010 G0/G1/G2 + EXT-UPD-4.8 + SWIM Reactive-0 + SWIM Reactive2 + IT-G1 + C10C-002 + C10C-001 + C10C-003 + C10C-004 + MT4 | SWIM provides bounded accessibility operationalization; IT-G1 adds bounded evidence that end-to-end accessibility/function can depend on conditions external to the internal remediation target; C10C-002 adds an independent real-world bounded reconstruction of `T_acc*` from six structural infrastructure dimensions using pre-outcome structural predicates across 342 polygons and two observed rounds. C10C-001 independently demonstrates in a distinct non-software case that structural state reconstruction and observed configuration changes do not identify `Pτ`/`T_acc` without an explicit admissibility predicate. C10C-003 adds a distinct randomized non-software boundary case in which rich longitudinal structural data and treatment/take-up variables do not identify a non-circular accessibility predicate within the bounded operational search. C10C-004 adds no direct positive accessibility evidence. MT4 provides a partially formalized technical candidate admissibility layer (`CORE6`) with 92.58% complete country-technology-year coverage, demonstrating bounded operationalization/auditability of the admissibility distinction in a new domain, but full `P_tau` remains undetermined. General `T_acc` remains unclosed. No claim-level upgrade is implied. | Independent operationalization across a distinct exemplar |
| C03 | T_acc is analytically distinct from downstream Reach in bounded Rust | E1 | Rust evidence | SWIM does not alter the bounded Rust claim level; IT-G1 does not test the Rust-specific Reach separation; C10C-002 does not operationalize downstream Reach identity; C10C-003 does not test Reach identity; MT4 does not test Reach identity. | Independent replication |
| C04 | ΔT_acc can occur without ΔReach¹_pot | E1 | Rust ND-1 | No direct test by SWIM, IT-G1, C10C-002, C10C-003 or MT4. | Independent replication |
| C05 | ΔT_acc can occur with ΔReach¹_pot change | E1 | Rust ND-2 | No direct test by SWIM, IT-G1, C10C-002, C10C-003 or MT4. | Independent replication |
| C06 | Reach identity is not characterized by cardinality alone | E1 | Rust ND-4 | No material impact; C10C-002 uses a scalar net cardinality outcome only for its frozen bounded causal test and does not establish Reach identity; C10C-003 does not test Reach identity; MT4 does not test Reach identity. | Independent replication |
| C07 | Accessible transformation spaces change over time in Rust | E1 | Rust non-persistent pairs + bounded SWIM operationalization + C10C-002 + C10C-001 + C10C-003 + C10C-004 + MT4 | SWIM provides bounded non-Rust evidence that accessible transformation spaces can change across observed state transitions; IT-G1 adds a bounded industrial state-transformation sequence; C10C-002 adds independent real-world longitudinal evidence from 342 polygons in which a bounded universe of 12 elementary structural transformations yields non-empty `ΔT_acc*` in 238 polygons. C10C-001 and C10C-003 qualify the distinction between observed structural change and accessible transformation-space change and are not evidence of `ΔT_acc`. C10C-004 does not reconstruct `U_tau`, `P_tau` or `ΔT_acc`. MT4 adds a new-domain bounded methodological separation between candidate constraints and realized technology changes, but does not identify a complete `T_acc` or positive `ΔT_acc` result. This does not upgrade the Rust-specific claim level. | Independent closed operationalization / replication |
| C08 | Accessibility changes modify reachable future trajectories | H | Formal chain + bounded Rust H=1 + SWIM trajectory-linkage reconstruction + IT-G1 bounded state/trajectory observation + C10C-001 + C10C-003 + MT4 | SWIM adds bounded reconstructability of ordered subsequent transformations and state transitions; IT-G1 provides a bounded industrial state/trajectory observation. KGFS supplies the causal layer now reflected in C09. C10C-001 and C10C-003 add complementary empirical boundary qualifications. MT4 establishes downstream separation of candidate constraints, realized transformations and state/trajectory variables, but does not establish a trajectory causal estimand. No claim-level upgrade is implied. | Independent valid trajectory test with explicit trajectory criterion beyond bounded exemplars |
| C09 | Accessibility changes causally affect subsequent trajectories | **PASS — BOUNDED EMPIRICAL CAUSAL SUPPORT** | SWIM trajectory-linkage reconstruction + RUST-DYN-2 bounded structural evidence + KGFS randomized structural accessibility intervention with D5-A identified contribution + KGFS exact 74/74 trajectory-variable reproducibility audit + C09 Bundle 003 Executor-2 reconstruction + FOS methodological/reference evidence | Claim-level consolidation establishes bounded empirical causal support. KGFS provides the decisive real-world causal layer. MT4 contains no causal estimate of `ΔT_acc → trajectory` and therefore provides no positive C09 propagation; it does not alter the existing bounded causal-support status. | Independent real-world replication across a distinct domain; broader generality; no automatic value claim |
| C10 | Accessibility changes generate/predict value | H | No Value evidence + MT4 value-isolation boundary | SWIM, RUST-DYN-2, KGFS, IT-G1, C10C-002 and C10C-003 do not establish causal `ΔT_acc → ΔV` or predictive value. MT4-8 structurally isolates `Inv`, `Fixed_OM_annual` and `Variable_OM` from the CORE6 candidate accessibility rule, but does not establish those variables as downstream TGCV value outcomes and does not test `ΔT_acc → ΔV`. C10 remains open. | Value-linked test |
| C11 | TGCV is domain-independent / transversal | H | C-01 A-C + I-01 Gate C + bounded cross-domain evidence + C10C-002 + C10C-001 + C10C-003 + MT4 | MT4 adds a genuinely heterogeneous historical electricity-system domain and strengthens the documented record of methodological transfer and boundary preservation. It does not establish transversal validity or justify a claim upgrade. | Independent operationalization across broader domains |
| C12 | TGCV provides superior explanatory representation | H | Comparative evidence absent | IUT M1 provides bounded decision-performance evidence but U2-NULL; SWIM, IT-G1, KGFS, C10C-002, C10C-003 and MT4 are non-comparative for explanatory superiority. | Controlled differentiated comparison |
| C13 | TGCV contains no equivalent prior architecture | O | D-OPS-21 | No material impact. | Comparative coverage |
| C14 | T_acc is an ontological primitive independent of S | F | TR-131 | No impact; SWIM, IT-G1, KGFS, C09 Bundle 003, C10C-002, C10C-003 and MT4 treat accessibility as derived/conditioned rather than restoring the rejected primitive. | No restoration without contrary evidence |
...
## Material methodological evidence — MT4 historical national electricity-system transitions
...
**Result artifact:** `00_GOVERNANCE/SIP/TGCV_MT4_DOMAIN_TRANSFER_RESULT_001.md`  
**Propagation record:** `00_GOVERNANCE/SIP/TGCV_MT4_EVIDENCE_TO_CLAIM_PROPAGATION_001.md`  
**Primary source:** Jaxa-Rozen, Wen & Trutnevyte, `Historic data of the national electricity system transitions in Europe in 1990–2019 for retrospective evaluation of models`, Zenodo record 6696776, v2.

MT4 transfers the candidate transversal methodology to a genuinely heterogeneous historical electricity-system domain while preserving explicit analytical distinctions among state/conditions, candidate technical constraints, admissibility/accessibility, realized transformations, state/trajectory and economic/value-related variables. The frozen public dataset covers 31 European countries and 1990–2019, with 1,359 processed/harmonized data files and 528 original data sources/references. The local frozen source package is governed independently by the MT4 source-freeze manifest.

### Gate results

- **MT4-1:** PASS — source discovery/provenance.
- **MT4-2:** PASS — source freeze/integrity.
- **MT4-3:** PASS — structural/schema reconstruction.
- **MT4-4:** PASS — bounded state/candidate-transformation/constraint coverage, with explicit temporal limitations.
- **MT4-5:** OPEN / BOUNDED — full `P_tau` remains undetermined. Temporal auditors identified retrospective/future-leakage risks in some `Buildrates`/`Potential_*` records; CORE6 coverage is 92.58%; the candidate technical rule is only partially formalized.
- **MT4-6:** PASS — independent reproducibility of the structural audit: 13,320 country-technology-year cells, 12,331 complete CORE6 cells, 989 incomplete cells, 92.58% coverage; 930 multi-technology country-year groups; one duplicate CORE6 signature group, without loss of discriminability across groups.
- **MT4-7:** BOUNDED PASS — candidate technical parameters remain separated from realized transformations (`Actual_new_capacity`, `Actual_retired_capacity`), state/trajectory variables (`Actual_capacity`, `Actual_generation`) and economic/value-related variables (`Inv`, `Fixed_OM_annual`, `Variable_OM`); candidate-rule/actual overlap violations = 0.
- **MT4-8:** BOUNDED PASS — economic/value-related variables are structurally isolated from the CORE6 candidate accessibility rule; no downstream TGCV `Delta V` endpoint is established and no causal value effect is tested.

### Methodological significance

MT4 provides bounded evidence that the candidate translation sequence can be applied in a non-software electricity-system domain without collapsing candidate constraints into realized technology changes or downstream economic/value-related variables. CORE6 provides a partially formalized candidate admissibility layer with 92.58% complete country-technology-year coverage, but full `P_tau` is not closed. The result therefore demonstrates methodological transfer and boundary preservation, not transversal validity or scientific closure.

### Evidence-to-claim propagation

- **C01:** bounded methodological support. MT4 demonstrates identification/preservation of a technical constraint/state layer in a heterogeneous electricity-system domain. No universal state representation or claim upgrade.
- **C02:** bounded qualification. CORE6 provides a partially formalized candidate admissibility layer with 92.58% complete coverage, but full `P_tau` remains undetermined. No claim upgrade.
- **C07:** boundary qualification only. MT4 distinguishes realized technology changes from accessibility-space change and does not substitute `Actual_*` variables for `T_acc` or `Delta T_acc`. No positive `Delta T_acc` propagation.
- **C08:** bounded methodological support only. MT4 separates candidate constraints, realized transformations and state/trajectory variables, but does not establish a trajectory causal estimand. No upgrade.
- **C09:** no positive propagation. MT4 contains no causal estimate of `Delta T_acc → trajectory`; existing C09 bounded empirical causal-support status remains unchanged.
- **C10:** material boundary qualification. MT4-8 isolates `Inv`, `Fixed_OM_annual` and `Variable_OM` from the CORE6 candidate accessibility rule, but does not establish them as downstream TGCV value outcomes. No `Delta T_acc → Delta V` evidence.
- **C11:** bounded methodological cross-domain evidence. MT4 adds a genuinely heterogeneous electricity-system domain and strengthens documented methodological transfer, but does not establish transversal validity.
- **C16:** material positive methodological evidence, bounded. MT4 preserves separation among state, candidate transformations, admissibility/accessibility, realized transformation, trajectory and economic/value-related variables. No upgrade.

### Explicit non-propagation

No positive evidence is propagated to C03, C04, C05, C06, C12, C13, C14 or C15. MT4 does not modify TGCV Core, RMA, claim levels, causal status or transversal-validity status. Evidence propagation remains separate from claim upgrade.

## Claim boundary

The v1.12 update preserves the full evidence/claim structure of v1.11 and adds MT4 as a bounded methodological domain-transfer evidence record. MT4 propagates bounded methodological support/qualification to C01, C02, C07, C08, C10, C11 and C16; it provides no positive propagation to C09 and no material impact to C03, C04, C05, C06, C12, C13, C14 or C15. No C01–C16 status is upgraded by MT4. The TGCV Core remains unchanged.
...
- G3 Trajectory sufficiency: OPEN; bounded trajectory-linkage and IT-G1 state-transition observations do not close the general gate; C10C-002, C10C-003 and MT4 do not establish trajectory causality.
- G4 Causal identification: **BOUNDED PASS at C09 claim level** through KGFS D5-A; C10C-002 provides a separate negative bounded causal test of intervention → `Delta T_acc*`; C10C-003 adds no positive causal identification; MT4 adds no causal identification.
- G5 Value linkage: OPEN; C10C-002 value pathway not executed, C10C-003 does not identify the accessibility layer required for a value pathway, and MT4-8 isolates value-related inputs without establishing a downstream value endpoint.
- G6 Originality/comparative architecture: BOUNDED / PARTIAL.
- G7 Transversal translation protocol: BOUNDED operational support; C10C-002, C10C-003 and MT4 add distinct non-software domains but general/transversal closure remains OPEN.
