# TGCV — Evidence-to-Claim Matrix — Current v1.17

**Status:** GOVERNANCE CONTROL ARTIFACT — CURRENT  
**Date:** 2026-09-18  
**Predecessor:** v1.16  
**Incremental governance update:** Adds the complete GL-07 material expediente for VSL Synthetic Minimum v0.1 and bounded methodological routing to C10 and C16. No claim-level upgrade; C09, Core and RMA remain unchanged.
...
| C10 | Accessibility changes generate/predict value | H | No Value evidence + MT4 + VSL Synthetic Minimum v0.1| SWIM, RUST-DYN-2, KGFS, IT-G1, C10C-002 and C10C-003 do not establish causal `ΔT_acc → ΔV` or predictive value. C10C-002 explicitly did not execute a value regression. C10C-003 does not identify the TGCV accessibility layer required for a value pathway. VSL Synthetic Minimum v0.1 adds bounded methodological evidence that an externally specified outcome-to-Value mapping can be operationalized separately from the transformation/accessibility path; it does not establish real-world Value or causal `Delta T_acc -> Delta V`. No claim-level upgrade. MT4-8 isolates `Inv`, `Fixed_OM_annual` and `Variable_OM` from the CORE6 candidate accessibility rule, but does not establish a downstream TGCV value endpoint or `Delta T_acc -> Delta V`. C10 remains open.| Value-linked test |
