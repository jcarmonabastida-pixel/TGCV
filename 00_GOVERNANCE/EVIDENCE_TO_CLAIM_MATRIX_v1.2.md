...
**Date:** 2026-09-11  
**Predecessor:** v1.1  
**Current update:** Material evidence propagation for SWIM Reactive-0 and Reactive2 bounded TGCV operationalization. No scientific claim status/level upgrade.
...
| C09 | Accessibility changes causally affect subsequent trajectories | H | No causal identification | No causal evidence from SWIM Reactive-0 or Reactive2. | Causal design |
...
| C16 | TGCV provides a transversal analytical translation protocol preserving distinctions among state, candidate transformations, accessibility, Reach, Trajectory, Outcome and Value | H | D-OPS-22/23 + C-01 A-C + I-01 Gate C + IT-NOSD-010 G0/G1/G2 + EXT-UPD-4.8 + SWIM Reactive-0 + SWIM Reactive2 | SWIM Reactive-0 adds a bounded operationalization of the state/context → accessibility → `T_acc` → `ΔT_acc` segment. Reactive2 adds bounded methodological evidence that candidate identity, accessibility and native policy selection can be represented as distinct analytical layers. It does not close downstream trajectory/value boundaries or establish transversal validity. | Closed independent-domain operationalization / downstream test |
...
No C01–C16 status is upgraded by the material evidence recorded in v1.2. The SWIM results strengthen the bounded evidentiary basis for C01, C02, C07 and C16; Reactive2 specifically adds bounded methodological evidence relevant to C02 and C16. These are material evidence records, not claim upgrades. The scientific Core remains unchanged.
...
The evidence base now includes bounded comparative methodological evidence from IUT-A-01 U2, bounded downstream-separation/reconstructability evidence from IT-NOSD-010, bounded accessibility-closure boundary evidence from EXT-UPD-4.8, Class-II AWS fixture evidence, and bounded self-adaptive software operationalization from SWIM Reactive-0 and Reactive2. These are material evidence records, not claim upgrades. The scientific Core remains unchanged.
