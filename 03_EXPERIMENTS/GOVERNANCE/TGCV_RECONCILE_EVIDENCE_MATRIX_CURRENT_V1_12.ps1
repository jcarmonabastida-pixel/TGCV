...
'@
$out=$current
# Avoid Unicode-sensitive header matching: replace the version token only.
$out=[regex]::Replace($out,'Current v1\.11','Current v1.12',1)
$out=[regex]::Replace($out,'\*\*Predecessor:\*\* v1\.10','**Predecessor:** v1.11',1)
$out=[regex]::Replace($out,'\*\*Incremental governance update:\*\*.*?(?=\r?\n\r?\n)','**Incremental governance update:** v1.12 preserves the complete material evidentiary content and schema of v1.11 and adds the complete MT4 domain-transfer evidence layer. No prior evidence is deleted, collapsed, or downgraded. No claim-level status is upgraded by MT4 propagation.',1)
if($out -match 'MT4 Domain Transfer'){ throw 'MT4_ALREADY_PRESENT_IN_CURRENT' }
...
$out=[regex]::Replace($out,'The v1\.10 update preserves the full evidence/claim structure of v1\.9','The v1.12 update preserves the full evidence/claim structure of v1.11 and adds the complete MT4 domain-transfer evidence layer',1)
$out=[regex]::Replace($out,'v1\.11 adds C10C-004 bounded methodological evidence propagation\. None','v1.11 adds C10C-004 bounded methodological evidence propagation, and v1.12 adds MT4 bounded domain-transfer evidence propagation. None',1)
...
if($check -notmatch 'Current v1\.12'){ throw 'MATRIX_VALIDATION_MISSING: Current v1.12' }
foreach($needle in @('MT4-1','MT4-8','92.58%','MT4-6 — independent reproducibility','MT4-7 — downstream separation','MT4-8 — value isolation','CORE6_VALUE_OVERLAP_VIOLATIONS=0','No TGCV Core/RMA modification is authorized by MT4')){ if($check.IndexOf($needle) -lt 0){ throw ('MATRIX_VALIDATION_MISSING: '+$needle) } }
