...
try{foreach($e in $a.Entries){if($e.FullName -notmatch '/Technologies/.*\.csv$'){continue};$r=[IO.StreamReader]::new($e.Open());try{while(($l=$r.ReadLine())-ne$null){if([string]::IsNullOrWhiteSpace($l)){continue};$x=Parse $l;if($x.Count-lt 10){continue};$country=Clean $x[0];$tech=Clean $x[1];$par=Clean $x[2];$yr=Clean $x[3];$v=Clean $x[5];if($core-notcontains$par){continue};if($yr-notmatch '^\d{4}$'){continue};if([string]::IsNullOrWhiteSpace($country)-or[string]::IsNullOrWhiteSpace($tech)){continue};$k="$country|$tech|$yr";if(!$cells.ContainsKey($k)){$cells[$k]=@{}};$cells[$k][$par]=$v}}finally{$r.Dispose()}}}finally{$a.Dispose()}
