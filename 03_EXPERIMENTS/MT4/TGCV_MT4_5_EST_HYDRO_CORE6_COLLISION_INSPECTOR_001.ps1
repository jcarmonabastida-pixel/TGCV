...
try{foreach($e in $z.Entries){if($e.FullName -notmatch '/Technologies/(HydroRoR|HydroDam)\.csv$'){continue};$tech=if($e.FullName -match '/(HydroRoR|HydroDam)\.csv$'){$Matches[1]}else{continue};$r=[IO.StreamReader]::new($e.Open());try{while(($l=$r.ReadLine())-ne$null){$x=Parse $l;if($x.Count-lt 10){continue};$par=Clean $x[2];$year=Clean $x[3];$value=Clean $x[5];if($year-ne'1990'){continue};if(!$rows.ContainsKey($tech)){$rows[$tech]=@{}};$rows[$tech][$par]=$value}}finally{$r.Dispose()}}}finally{$z.Dispose()}
...
foreach($tech in @('HydroRoR','HydroDam')){$lines.Add("TECHNOLOGY=$tech");if($rows.ContainsKey($tech)){foreach($p in ($rows[$tech].Keys|Sort-Object)){$lines.Add("$p=$($rows[$tech][$p])")}}else{$lines.Add('NO_RECORDS_FOUND')};$lines.Add('')}
