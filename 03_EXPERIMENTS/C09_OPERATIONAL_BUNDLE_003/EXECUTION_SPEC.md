# TGCV C09 Operational Execution Bundle 003
...
**Freeze scope:** operational definitions, fixture, randomization, null control, endpoint, executor contract, integrity verification and reconstruction boundary.
...
A predeclared null run uses the same units, assignment and frozen generator but sets R1=false for both arms. It is a measurement/control check only and is not part of the primary treatment effect. A non-zero null contrast is recorded as a control observation and is not, by itself, an integrity failure.

## 7. Integrity checks

The executor must verify:
1. `T_acc,0 != T_acc,1` and exact expected values;
2. baseline S0/C0/U definitions are frozen and primary/null rows use the same units, assignments and baseline values;
...
7. every output row has the declared canonical field set and the emitted JSON uses deterministic canonical serialization;
8. SHA-256 of `fixture.json`, `EXECUTION_SPEC.md`, and the executed `execute_c09_bundle_003.py` exactly matches `HASH_MANIFEST.md` before any scientific estimate is authorized.

Any failed check yields `BLOCKED` and no scientific estimate is emitted.

The null contrast is reported but is not required to equal zero because the same randomized units may have different baseline means between arms.
