...
def manifest_hashes(root):
    ...
    for line in manifest.splitlines():
        ...
        if len(parts) == 2 and parts[0] in EXPECTED_FILES:
...
def integrity_checks(primary, null, assignment_map, root):
    ...
    checks["accessibility_exact"] = t_acc(False) == ["A", "C"] and t_acc(True) == ["A", "B", "C"] and t_acc(False) != t_acc(True)
    ...
    checks["bundle_sha256_matches_manifest"] = declared == actual and set(declared) == set(EXPECTED_FILES)
...
def main():
    ...
    checks, hashes = integrity_checks(primary, null, ASSIGNMENT, root)
    ...
    result = {
        ...
        "accessibility_control": t_acc(False), "accessibility_treatment": t_acc(True),
        "rows": primary,
    }
