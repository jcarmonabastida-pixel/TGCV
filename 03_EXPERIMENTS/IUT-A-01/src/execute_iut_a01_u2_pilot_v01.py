#!/usr/bin/env python3
"""IUT-A-01 U2 decision-performance computational pilot.
...
EXECUTOR_VERSION = "IUT-A-01-U2-PILOT-EXECUTOR-0.1"
CASE_ID = "IUT-A-01"
N_TRIALS = 40
TRIAL_CLASSES = (
    "DIRECT_FEASIBILITY",
...
def build_trial(index: int, trial_class: str) -> Trial:
    # Four deterministic patterns. The same underlying facts feed both arms.
    cycle = index % 5
...
def hash_trials(trials: tuple[Trial, ...]) -> str:
    return sha256_text(canonical_json(trial_payload(trials)))
...
def control_decision(trial: Trial) -> str:
    """Conventional bounded procedure: scans static option descriptions in order.

    It checks direct requirements and hard constraints, but does not construct an
    explicit relation between dependency conditions and future accessibility.
    In this fixture, dependency-bearing alternatives are therefore considered only
    when all dependency facts are directly attached to the option description.
    """
    ...
    for opt in trial.options:
        ...
        if opt.dependencies:
            # Baseline treats dependency conditions conservatively as unavailable.
            dependency_ok = False
        else:
            dependency_ok = True
...
def tgcv_decision(trial: Trial) -> str:
    """TGCV-assisted procedure using explicit accessibility relation."""
    state = state_map(trial)
...
def decision_time_control(trial: Trial) -> float:
    # Deterministic synthetic proxy for decision effort; not wall-clock measurement.
    state = state_map(trial)
    ...
    for opt in trial.options:
        ...
        if opt.dependencies:
            # baseline incurs extra uncertainty penalty for dependency alternatives
            checks += 3
    return 1.0 + 0.12 * complexity + 0.04 * checks + 0.18 * (1 if trial.trial_class in {"DEPENDENCY", "ALTERNATIVE_SPACE"} else 0)


def decision_time_tgcv(trial: Trial) -> float:
    # Explicit accessibility mapping reduces repeated dependency reasoning.
...
def score(trials: tuple[Trial, ...]) -> dict:
    ...
    for trial in trials:
        c0 = control_decision(trial)
        c1 = tgcv_decision(trial)
        ...
        t0 = decision_time_control(trial)
        t1 = decision_time_tgcv(trial)
        ...
        records.append({
            ...
            "control_time_proxy": round(t0, 6),
            "tgcv_time_proxy": round(t1, 6),
        })
    ...
    return {
        ...
        "M2": {
            "control_median_time_proxy": m2_control,
            "tgcv_median_time_proxy": m2_tgcv,
...
def integrity_checks(trials: tuple[Trial, ...]) -> dict:
...
def run(mode: str) -> dict:
    ...
    integrity = integrity_checks(trials)
    fixture_hash = hash_trials(trials)

    result: dict = {
        "EXECUTION_RESULT": "PASS" if all(integrity.values()) or mode == "DRY_RUN" else "INDETERMINATE",
        "MODE": mode,
