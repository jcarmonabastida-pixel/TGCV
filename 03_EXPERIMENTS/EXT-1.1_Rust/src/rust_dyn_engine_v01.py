#!/usr/bin/env python3
"""RUST-DYN deterministic downstream engine v0.1.

Synthetic-first implementation of the frozen RUST-DYN-1 semantics.
...
def _successors(origin_id: int, tacc_by_origin: TAccMap) -> tuple[int, ...]:
    """Return deterministic next-origin IDs from accessible transformations."""
...
def trajectories_h(origin_id: int, tacc_by_origin: TAccMap, horizon: int) -> tuple[tuple[int, ...], ...]:
    ...
    for _ in range(horizon):
        expanded = []
        for path in paths:
            successors = _successors(path[-1], tacc_by_origin)
            if not successors:
                expanded.append(path)
            else:
                for nxt in successors:
                    # Cycles are not extended indefinitely; retaining the prefix
                    # makes the finite-horizon path representation deterministic.
                    if nxt in path:
...
def synthetic_conformance() -> dict:
    # Small deterministic graph: 1 and 2 have different T_acc but identical
    # one-step Reach; 3 and 4 have equal Reach with different path structure.
    tau12 = (1, 10, 2, "1.0.0")
    tau13 = (1, 10, 3, "1.1.0")
    tau24 = (2, 20, 4, "1.0.0")
    tau34 = (3, 20, 4, "1.0.0")
    ...
    tests = {}
    d = delta_tacc((tau12,), (tau13,))
    tests["delta_reconfiguration_equal_cardinality"] = d["classification"] == "RECONFIGURATION" and d["changed"]
    tests["delta_persistence"] = delta_tacc((tau12,), (tau12,))["classification"] == "PERSISTENCE"
    tests["reach_deterministic"] = reach_h(1, graph, 1) == (1, 2)
