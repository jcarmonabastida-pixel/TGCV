#!/usr/bin/env python3
"""EXT-1.1 DR-027 Primary/Replay identity verifier v0.1.
...
def normalized_manifest(value: dict) -> dict:
    x = dict(value)
    # Each execution necessarily records its own HEAD and mode. These are
    # execution-instance metadata, not protocol identity.
    x.pop("mode", None)
    x.pop("git_head", None)
    return x
...
def main() -> int:
    print("TGCV EXT-1.1 — DR-027 Primary/Replay identity verifier v0.1")
    ...
    checks = {
        ...
        "MANIFEST_IDENTITY": normalized_manifest(p_man) == normalized_manifest(r_man),
    ...
    # Explicit critical-result equality, kept separate for auditability.
    ...
    prohibited_ok = all(v is False for v in {
    ...
    checks["PROHIBITED_POST_HOC_ACTIONS_FALSE"] = prohibited_ok
