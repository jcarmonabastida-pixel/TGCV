"""C09 OLPC Peru causal-gap closure executor v0.4.
...
def build_raven(root: Path):
    # input_r2.do explicitly uses Originales/Ronda2/matrices_g3-6_r2.dta,
    # then renames serie_a_01..serie_b_12 to p1..p36 and scores those items.
    path=locate(root,"matrices_g3-6_r2.dta")
    x,_=read_dta(path)
    cod=resolve_col(x,"codest")
    series=[*(f"serie_a_{i:02d}" for i in range(1,13)),*(f"serie_ab_{i:02d}" for i in range(1,13)),*(f"serie_b_{i:02d}" for i in range(1,13))]
    cols=[resolve_col(x,n) for n in series]
    ...
    out=pd.DataFrame({"codest":x[cod],"raven_r2":score})
    return out,path,"reconstructed_from_matrices_g3-6_r2_via_published_rename_and_key"
...
def main():
    ...
    paths={n:locate(root,n) for n in ["listas_final.dta","school_pairs_final.dta","cestudiante_g3-6_p2_r1.dta","cestudiante_g3-6_p2_r2.dta","matrices_g3-6_r2.dta"]}
    ...
    caps=[]
    for i in range(1,9): n=f"Tacc0_cap_A{i}"; r1[n]=binary_yes(r1[resolve_col(r1,f"P12_A{i}")]); caps.append(n)
    ...
    export=["codest","Z","received_laptop","treatment_school","Tacc0_capacity_count","Tacc0_resource_computer","Tacc0_resource_internet","Tacc0_resource_prior_use","Tacc1_resource_computer","Tacc1_resource_internet","Tacc1_resource_prior_use","delta_computer","delta_internet","delta_prior_use","delta_resource_count"]+trajectory_variables+["raven_r2"]; audit_path=out/"C09_OLPC_PERU_CAUSAL_GAP_CLOSURE_AUDIT_DATA_001.csv"; d[[c for c in export if c in d.columns]].to_csv(audit_path,index=False)
