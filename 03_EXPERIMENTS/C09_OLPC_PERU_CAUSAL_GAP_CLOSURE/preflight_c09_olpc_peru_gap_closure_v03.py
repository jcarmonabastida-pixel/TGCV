...
def main():
    checks={}
    required=[EXEC,G5,SPEC,AUDIT]
    checks["required_files"]={str(p):p.exists() for p in required}
    checks["required_files_pass"]=all(checks["required_files"].values())
    e=text(EXEC) if EXEC.exists() else ""
    g=text(G5) if G5.exists() else ""
    a=text(AUDIT) if AUDIT.exists() else ""
    ...
    checks["no_first_stage_only_verdict"]='elif not first_pass:' in e and 'else:\n        status="PARTIAL/INCONCLUSIVE"' in e
    ...
    checks["syntax_executor"] = False
    checks["syntax_g5"] = False
    ...
    out=BASE/"output_v07"; out.mkdir(exist_ok=True)
    rp=out/"C09_OLPC_PERU_CAUSAL_GAP_CLOSURE_PREFLIGHT_003.json"; rp.write_text(json.dumps(result,ensure_ascii=False,indent=2,sort_keys=True),encoding="utf-8")
