#!/usr/bin/env python3
import unittest
import TR131_CROSS_DOMAIN_COMPARISON_RUNNER_002 as R

def rec(rid="r",domain="TEST",t0=("a",),t1=("a",),real="a",tid=None,step=None):
    d={"domain":domain,"record_id":rid,"S_t":{"x":0},"T_acc_t":list(t0),"T_real_t":real,"S_t1":{"x":1},"T_acc_t1":list(t1)}
...
class T(unittest.TestCase):
    ...
    def test_missing_visible(self):
        ...
        p={"protocol":R.PROTOCOL,"records":[x]}; y=R.analyze(p); self.assertEqual(y["invalid_count"],1)
    ...
    def test_trajectory_divergence(self):
        a=R.derive(rec("a","D",("x",),("x","y"),tid="b1",step=0)); b=R.derive(rec("b","D",("x",),("x","y","z"),tid="b1",step=1))
        c=R.derive(rec("c","D",("x",),("x","z"),tid="b2",step=0)); d=R.derive(rec("d","D",("x",),("x",),tid="b2",step=1))
        y=R.trajectory_analysis([a,b,c,d]); self.assertEqual(y["D"]["divergence"],"OBSERVED")
    def test_domain_separation(self):
        p={"protocol":R.PROTOCOL,"records":[rec("v","VisitAll"),rec("p","PRISM",("pick",),("read",),"pick")]}
        y=R.analyze(p); self.assertEqual(y["domains"],["PRISM","VisitAll"])
    def test_deterministic_hash(self):
        p={"protocol":R.PROTOCOL,"records":[rec(t1=("a","b"))]}
