"""TGCV WP2 TSTC deterministic Fixture-002 engine.

Preflight/conformance only. This engine binds only to the frozen Fixture-002
scientific definition. It does not execute trajectories, baselines, outcomes,
or value analysis.
"""
...
VERSION = "TSTC_FIXTURE_ENGINE_v002"
...
def _c01():
    ...
    def tr(name):
        return {"c01.deploy_A":_compose(_state("service","deployed"),_state("compute_A","occupied")),"c01.deploy_B":_compose(_state("service","deployed"),_state("compute_B","occupied")),"c01.route_A_to_B":_state("routing","B"),"c01.route_B_to_A":_state("routing","A"),"c01.restrict_security":_state("security","restricted"),"c01.restore_security":_state("security","normal")}[name]
    ...
    names=tuple(p)
    ts=tuple(Transformation(n,"C01",p[n],av[n],tr(n)) for n in names)
    return Fixture("FX-C01","002",s,c,ts,"I-C01",_context("trust_B","untrusted"),("trust_B",),"N-C01",_state("routing","B"),("routing",),"finite-state-rule-graph")
...
def _c03():
    ...
    p={"c03.query_db":lambda s,c:s["db"]=="available" and c["permission_db"]=="granted" and c["tool_query"]=="available","c03.inspect_repo":lambda s,c:s["repo"] in ("clean","changed") and c["permission_repo"]=="granted","c03.open_pr":lambda s,c:c["permission_repo"]=="granted" and c["tool_pr"]=="available" and s["repo"] in ("clean","changed"),"c03.modify_repo":lambda s,c:s["repo"]=="clean" and c["permission_repo"]=="granted"}
    av={"c03.query_db":(),"c03.inspect_repo":(),"c03.open_pr":(),"c03.modify_repo":("repo",)}
    tr={"c03.query_db":_noop,"c03.inspect_repo":_noop,"c03.open_pr":_noop,"c03.modify_repo":_state("repo","changed")}
    ts=tuple(Transformation(n,"C03",p[n],av[n],tr[n]) for n in p)
    return Fixture("FX-C03","002",s,c,ts,"I-C03",_context("permission_repo","denied"),("permission_repo",),"N-C03",_context("tool_query","available"),("tool_query",),"capability-permission-matrix-workflow",("c03.complete_task",))
...
def _c05():
    ...
    return Fixture("FX-C05","002",s,c,ts,"I-C05",_state("grid_capacity","low"),("grid_capacity",),"N-C05",_context("mobility_requirement_A","normal"),("mobility_requirement_A",),"finite-constrained-feasibility")
...
def preflight_fixture(f):
    ...
    for t in f.transformations:
        assert t.affected_variables is not None
