...
class Fixture:
    ...
    declared_negative_control_variables: Tuple[str, ...]
    baseline_kind: str
...
def _c01():
    ...
    def pred(name):
        return {
            ...
            "c01.restore_security":lambda s,c:s["security"]=="restricted"}[name]
    def trans(name):
        def op(s,c):
    ...
    return Fixture("FX-C01","001",s,c,ts,"I-C01",_replace_context("trust_B","untrusted"),("trust_B",),"N-C01",_replace_state("routing","B"),("routing",),"finite-state-rule-graph")
...
def _c03():
    ...
    p={"c03.query_db":lambda s,c:s["db"]=="available" and c["permission_db"]=="granted" and c["tool_query"]=="available","c03.inspect_repo":lambda s,c:s["repo"] in ("clean","changed") and c["permission_repo"]=="granted","c03.open_pr":lambda s,c:c["permission_repo"]=="granted" and c["tool_pr"]=="available" and s["repo"] in ("clean","changed"),"c03.complete_task":lambda s,c:s["task"]=="pending"}
    ts=tuple(Transformation(n,"C03",p[n],(),_noop) for n in p)
    return Fixture("FX-C03","001",s,c,ts,"I-C03",_replace_context("permission_repo","denied"),("permission_repo",),"N-C03",_replace_context("tool_query","available"),("tool_query",),"capability-permission-matrix-workflow")
...
def _c05():
    ...
    return Fixture("FX-C05","001",s,c,ts,"I-C05",_replace_state("grid_capacity","low"),("grid_capacity",),"N-C05",_replace_context("mobility_requirement_A","normal"),("mobility_requirement_A",),"finite-constrained-feasibility")

def fixtures(): return (_c01(),_c03(),_c05())

def tacc(fixture,state=None,context=None):
    state=fixture.state if state is None else state; context=fixture.context if context is None else context
    out=[]; traces={}
    for t in fixture.transformations:
        result=bool(t.predicate(state,context)); traces[t.transformation_id]={"admissible":result,"domain":t.domain}
...
def apply_checked(state,context,operation,allowed_variables):
    before=(deepcopy(state),deepcopy(context)); after=operation(state,context)
    if not isinstance(after,tuple) or len(after)!=2: raise AssertionError("transition must return (state, context)")
    s1,c1=after; changed=[]
    ...
    if unexpected: raise AssertionError(f"undeclared mutation: {sorted(unexpected)}")
    return s1,c1,tuple(sorted(changed))

def fixture_manifest(f):
    return {"fixture_id":f.fixture_id,"fixture_version":f.fixture_version,"state":f.state,"context":f.context,"transformation_universe":[t.transformation_id for t in f.transformations],"intervention_id":f.intervention_id,"negative_control_id":f.negative_control_id,"baseline":f.baseline_kind}

def preflight_fixture(f):
    manifest=fixture_manifest(f); assert len(manifest["transformation_universe"])==len(set(manifest["transformation_universe"]))
    t0,_=tacc(f); assert set(t0)<=set(manifest["transformation_universe"])
    for t in f.transformations: assert bool(t.predicate(f.state,f.context))==(t.transformation_id in t0)
    s1,c1,changed=apply_checked(f.state,f.context,f.intervention,f.declared_intervention_variables); t1,_=tacc(f,s1,c1)
    delta=(tuple(sorted(set(t1)-set(t0))),tuple(sorted(set(t0)-set(t1))))
    ns,nc,nchanged=apply_checked(f.state,f.context,f.negative_control,f.declared_negative_control_variables)
    nt,_=tacc(f,ns,nc); assert set(nt)==set(t0), f"negative control changed T_acc: {set(t0)^set(nt)}"
    assert tacc(f)[0]==t0
    return {"status":"PREFLIGHT_PASS","fixture":manifest,"T_acc_0":t0,"T_acc_1":t1,"delta_T_acc":{"opened":delta[0],"closed":delta[1]},"intervention_changed":changed,"negative_control_changed":nchanged,"negative_control_delta":tuple(sorted(set(nt)^set(t0))),"ruleset_hash":digest(manifest),"output_hash":digest({"fixture":manifest,"t0":t0,"t1":t1})}
