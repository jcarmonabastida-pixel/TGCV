...
FORBIDDEN_RUNTIME_TERMS=['successors','successor_state','successor_accessibility']

def load(p): return json.loads(Path(p).read_text(encoding='utf-8'))
def sha(o): return hashlib.sha256(json.dumps(o,sort_keys=True,separators=(',',':'),ensure_ascii=False).encode()).hexdigest()
def result(checks,reasons): return {**checks,'overall_compatibility_pass':all(checks.values()),'reasons':reasons,'scientific_execution':'NOT_AUTHORIZED'}

def main(argv):
    p=argparse.ArgumentParser()
    p.add_argument('fixture'); p.add_argument('provider'); p.add_argument('executor'); p.add_argument('executor2'); p.add_argument('contract'); p.add_argument('runtime_freeze'); p.add_argument('output')
    ...
    checks['fixture_hash_pass']=sha(fixture)==EXPECTED_FIXTURE_SHA
    ...
    checks['executor_binding_pass']=EXPECTED_EXECUTOR in executor and EXPECTED_CONTRACT in executor and EXPECTED_FIXTURE_SHA in executor
    ...
    checks['executor2_hash_pass']=EXPECTED_FIXTURE_SHA in executor2
    checks['contract_binding_pass']=EXPECTED_FIXTURE_SHA in contract and EXPECTED_ESTIMAND in contract and EXPECTED_PROVIDER in contract and EXPECTED_EXECUTOR in contract
    ...
    checks['scientific_execution_absent_pass']=not any(t in (provider+executor+executor2) for t in ['client.responses.create(']) or '--execute' in provider
