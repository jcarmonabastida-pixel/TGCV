...
EXPECTED_FIELDS = ["decision_id", "pair_id", "condition", "presentation", "context", "available_actions", "future_structure"]
FORBIDDEN_KEYS = {"utility", "reward", "value", "performance", "task_success", "successor_state", "outcome", "model_response", "scientific_score"}
...
def main():
    ...
    fixture, generator, schema = Path(args.fixture), Path(args.generator), Path(args.schema)
    fixture_bytes = fixture.read_bytes()
    data = json.loads(fixture_bytes.decode("utf-8"))
    units = data.get("decision_units", [])
    pairs, first_counts = {}, {c: {"I1_FIRST": 0, "I2_FIRST": 0} for c in EXPECTED_CONDITIONS}
    condition_units, presentation_units = {c: 0 for c in EXPECTED_CONDITIONS}, {p: 0 for p in EXPECTED_PRESENTATIONS}
    decision_ids = []
    for unit in units:
        decision_ids.append(unit.get("decision_id"))
        condition, presentation, pair_id = unit.get("condition"), unit.get("presentation"), unit.get("pair_id")
    ...
    checks["A12_PAIR_PRESENTATIONS"] = all(sorted(u.get("presentation") for u in v) == ["I1_FIRST", "I2_FIRST"] for v in pairs.values())
    checks["A13_PAIR_CONDITION_BALANCE"] = {c: sum(1 for v in pairs.values() if v[0].get("condition") == c) for c in EXPECTED_CONDITIONS} == EXPECTED_CONDITIONS
    ...
    checks["A16_FIRST_ORIENTATION_BALANCE"] = first_counts == {c: {"I1_FIRST": 35, "I2_FIRST": 35} for c in EXPECTED_CONDITIONS}
    checks["A17_DECISION_IDS_UNIQUE"] = len(decision_ids) == len(set(decision_ids))
    checks["A18_PAIR_IDS_UNIQUE"] = len(pairs) == EXPECTED_PAIRS
    ...
    checks["A21_FUTURE_STRUCTURE_SEMANTICS"] = all(u.get("future_structure") == {"successor_realized": False, "future_structure_available": u.get("condition") == "treatment"} for u in units)
    checks["A22_NO_FORBIDDEN_FIELDS"] = not any(key in FORBIDDEN_KEYS for u in units for key in u.keys())
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        subprocess.check_call([sys.executable, str(generator), "--output", str(reconstructed)], stdout=subprocess.DEVNULL)
        checks["A25_DETERMINISTIC_RECONSTRUCTION"] = reconstructed.read_bytes() == fixture_bytes
    status = "PASS" if all(checks.values()) else "FAIL"
    result = {"gate_id": "TI001-V011-FIXTURE-INTEGRITY-GATE-001", "status": status, "checks": checks, "fixture_id": EXPECTED_FIXTURE_ID, "schema_id": EXPECTED_SCHEMA_ID, "generator_id": EXPECTED_GENERATOR_ID, "seed": EXPECTED_SEED, "decision_count": len(units), "pair_count": len(pairs), "fixture_sha256": sha256_bytes(fixture_bytes), "generator_sha": git_blob_sha(generator), "schema_sha": git_blob_sha(schema), "scientific_execution": "NOT_PERFORMED"}
