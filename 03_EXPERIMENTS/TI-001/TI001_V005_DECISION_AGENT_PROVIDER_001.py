#!/usr/bin/env python3
"""TI-001 v005 decision provider.

v005-scoped infrastructure. This implementation does not reuse the v004
provider schema or successor contract. Scientific calls occur only with
--execute and after a separate v005 authorization gate.
"""
...
BACKGROUND = False
STORE = False
PROVIDER_VERSION = "TI001_DECISION_AGENT_PROVIDER_V005_001"
ALLOWED = {"a", "b", "c"}
...
def canonical_fixture_hash(fixture: dict) -> str:
    return canonical_json_hash(fixture)
...
def validate_decision_units(units: list, fixture: dict) -> None:
    if not units:
    ...
    for u in units:
        ...
        if u.get("condition") == "null" and u.get("null_semantic") not in (None, "null"):
...
def main(argv: list[str]) -> int:
    ...
    fixture_hash = canonical_fixture_hash(fixture)
    validate_decision_units(units["decision_units"], fixture)
    ...
    for unit in units["decision_units"]:
        ...
        decisions[key] = {
            ...
            "generation_configuration": {
                ...
                "conversation": None, "reasoning": None, "store": STORE
            },
    ...
    package = {
        "record_type": "TGCV_TI001_V005_DECISION_AGENT_OUTPUT",
        "provider_version": PROVIDER_VERSION,
        "fixture_sha256": fixture_hash,
        "base_prompt_sha256": prompt_hash,
        "model_id": MODEL_ID,
