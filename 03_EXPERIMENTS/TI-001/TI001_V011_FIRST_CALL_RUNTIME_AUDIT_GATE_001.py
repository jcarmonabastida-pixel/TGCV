...
EXPECTED_VERSION = "TI001-V011-SCIENTIFIC-EXECUTOR-001"
...
def main():
    ...
    checks["A7_EXECUTOR_VERSION_EXACT"] = 'EXECUTOR_VERSION = "TI001-V011-SCIENTIFIC-EXECUTOR-001"' in source
    checks["A8_MODEL_EXACT"] = 'MODEL_ID = "gpt-5.6-luna"' in source
    checks["A9_GENERATION_CONFIGURATION_EXACT"] = all(x in source for x in [
        "TOP_P = 0.98", "MAX_OUTPUT_TOKENS = 64", "TOOLS = []",
        'TOOL_CHOICE = "auto"', "BACKGROUND = False", "STORE = False"
    ]) and '"reasoning": None' in source
    calls = [n for n in ast.walk(tree) if isinstance(n, ast.Call) and isinstance(n.func, ast.Attribute) and n.func.attr == "create" and isinstance(n.func.value, ast.Name) and n.func.value.id == "responses"]
    checks["A10_EXACTLY_ONE_RESPONSES_CREATE_CALL"] = len(calls) == 1
    checks["A11_NO_RETRY_MECHANISM"] = not any(token in source.lower() for token in ("retry", "tenacity", "backoff"))
    checks["A12_NO_RECODE_REPAIR_INFERENCE_IMPUTATION"] = not any(token in source.lower() for token in ("recode", "repair", "imputation", "infer("))
    checks["A13_PAYLOAD_FROM_DECISION_PROJECTION"] = 'input=json.dumps(' in source and 'model_input["decision"]' in source
    checks["A14_HIDDEN_FIELDS_NOT_TRANSMITTED"] = all(field not in 'model_input["decision"]' for field in ("decision_id", "pair_id", "condition", "presentation"))
    checks["A15_VISIBLE_FIELDS_EXACT"] = all(field in source for field in ("context", "available_actions", "future_structure"))
    ...
    checks["A18_NO_FORBIDDEN_SCIENTIFIC_FIELDS_TRANSMITTED"] = all(field not in source.split('input=json.dumps(',1)[1].split('),',1)[0] for field in ("reward", "utility", "performance", "task_success", "successor_realized"))
    ...
    result = {
        "gate_id": "TI001-V011-FIRST-CALL-RUNTIME-AUDIT-GATE-001",
        "status": "PASS" if all(checks.values()) else "FAIL",
        "checks": checks,
        "executor_blob_sha": blob_sha(EXECUTOR),
        "fixture_sha256": sha256_file(FIXTURE),
        "scientific_execution": "NOT_PERFORMED",
        "authorization": "READY_FOR_FIRST_CALL"
    }
