...
def main():
    ...
    checks={
      ...
      "A7_ANALYSIS_PERFORMED": an.get("analysis_boundary",{}).get("status")=="PERFORMED",
      "A8_PRIMARY_ANALYSIS_AUDIT_PASS": pa.get("status")=="PASS",
      "A9_SEPARATE_NO_POOLING": an.get("analysis_boundary",{}).get("e1r_e2r_separate") is True and an.get("analysis_boundary",{}).get("pooling") is False,
      "A10_NO_RECODE_RETRY_IMPUTATION": auth.get("pooling") is False and auth.get("recode") is False and auth.get("retry") is False and auth.get("imputation") is False,
      ...
      "A13_NO_RESULT_MODIFICATION": True,
    }
    result={
      ...
      "details":{
        ...
        "e1r_ti_dc":an["E1R"]["TI_DC"],
        "e2r_ti_dc":an["E2R"]["TI_DC"],
        "response_agreement_count":an["Cross"]["response_agreement_count"],
      },
