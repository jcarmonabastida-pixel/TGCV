...
from branch_n_r_v02 import State, apply_transformation, enumerate_transformations
...
def goal(state: State, objective: str) -> bool:
    if objective == "O01": return "A1" in state.components
    if objective == "O02": return "A2" in state.components
    if objective == "O03": return "B1" in state.components
    if objective == "O04": return "B2" in state.components
    if objective == "O05": return "C1" in state.components
    if objective == "O06": return "C2" in state.components
    if objective == "O07": return state.resources[0] == 3
    if objective == "O08": return state.resources[1] == 3
    if objective == "O09": return state.resources[2] == 3
    if objective == "O10": return state.resources[0] == 0
    if objective == "O11": return state.resources[1] == 0
    if objective == "O12": return state.resources[2] == 0
...
def canonical_state_bytes(state: State) -> bytes:
    obj = {"components": list(state.components), "edges": [list(e) for e in state.edges],
           "objective": state.objective, "resources": list(state.resources)}
...
def transformation_id(t: tuple) -> str:
    return repr(t)
...
def generate_trajectory(state: State, dataset_split: str, dataset_seed: int, episode_id: int) -> TrajectoryRecord:
    ...
    for h in range(HORIZON + 1):
        if goal(current, state.objective):
            return TrajectoryRecord(episode_id, dataset_split, dataset_seed, seed,
        ...
        records.append(StepRecord(h, before, transformation_id(selected), state_sha256(nxt)))
        current = nxt
...
def canonical_trajectory_json(record: TrajectoryRecord) -> bytes:
    obj = {
        "dataset_seed": record.dataset_seed, "dataset_split": record.dataset_split,
        "episode_id": record.episode_id, "horizon": record.horizon,
        "initial_snapshot_sha256": record.initial_snapshot_sha256,
        "objective": record.objective, "outcome": record.outcome,
        "steps": [{"state_sha256_after": s.state_sha256_after,
                    "state_sha256_before": s.state_sha256_before,
                    "step": s.step, "transformation_id": s.transformation_id}
                   for s in record.steps],
        "terminal_reason": record.terminal_reason, "terminal_step": record.terminal_step,
...
def canonical_trajectory_jsonl(records: Iterable[TrajectoryRecord]) -> bytes:
    return b"".join(canonical_trajectory_json(r) for r in sorted(records, key=lambda r: r.episode_id))
